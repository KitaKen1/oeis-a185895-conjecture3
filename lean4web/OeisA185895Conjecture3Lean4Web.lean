...
/-!
# OEIS A185895: standalone Lean4Web target

This mathlib-only file mirrors the definitions and exact Gauss congruence target from
`FormalConjectures/OEIS/185895.lean` at commit
`e13dd7284e72012a1616806d09cb6b8025e387af`.

The mirrored definitions originate in the Formal Conjectures project and are used under its
Apache-2.0 license.  The proof below is self-contained over mathlib: in particular it does not
import or invoke the upstream open conjecture.
-/

open Polynomial

namespace OeisA185895Conjecture3Lean4Web

noncomputable section

/-- The finite polynomial approximation `∏ k ∈ [1,n], (1 - X^k / k!)`. -/
noncomputable def P (n : ℕ) : Polynomial ℚ :=
  ∏ k ∈ Finset.Icc 1 n, (1 - C (1 / (k.factorial : ℚ)) * X ^ k)

/-- The signed sequence with exponential generating function `∏_{k≥1} (1 - x^k/k!)`. -/
...
lemma P_eq_subset_sum (n : ℕ) :
    P n = ∑ s ∈ (Finset.Icc 1 n).powerset,
      C ((-1 : ℚ) ^ s.card * ∏ k ∈ s, (1 / (k.factorial : ℚ))) *
        X ^ (∑ k ∈ s, k) := by
  classical
  rw [P, Finset.prod_sub]
...
lemma coeff_P (n N : ℕ) :
    coeff (P n) N = ∑ s ∈ (Finset.Icc 1 n).powerset,
      if (∑ k ∈ s, k) = N then
...
/-- The signed multinomial sum indexed by distinct positive part sizes. -/
def explicit (N : ℕ) : ℤ :=
...
lemma coeff_mul_factorial_eq_cast_explicit (N : ℕ) :
    coeff (P N) N * (N.factorial : ℚ) = (explicit N : ℚ) := by
  rw [coeff_P]
  rw [Finset.sum_mul]
...
theorem a_eq_explicit {N : ℕ} (hN : N ≠ 0) : a N = explicit N := by
  rw [a, if_neg hN, coeff_mul_factorial_eq_cast_explicit]
  simp

/--
The proposition of the standalone mirror of `OeisA185895.conjecture3`.
The theorem `conjecture3Target_solved` below inhabits this proposition.
-/
def conjecture3Target : Prop :=
  ∀ (p : ℕ), p.Prime → ∀ (n k : ℕ), 0 < n → 0 < k →
    a (n * p ^ k) ≡ a (n * p ^ (k - 1)) [ZMOD (p : ℤ) ^ k]

#check conjecture3Target
#print axioms conjecture3Target

end

end OeisA185895Conjecture3Lean4Web

/-! ## Prime-power reduction of multinomial coefficients -/

open scoped BigOperators

namespace A185895Alt
...
lemma multinomial_finset_prime_reduction_zmod
  ...
  by_cases hd : ∀ x ∈ s, p ∣ x
  · rw [if_pos hd]
    rw [if_pos (fun x : s ↦ hd x x.property)] at h
...
lemma explicit_eq_signedShapeSum (N : ℕ) :
  ...
  classical
  simp only [explicit, signedShapeSum, shapes, Finset.sum_filter]

/-- The `research open` Formal Conjectures target, proved without importing that theorem. -/
theorem conjecture3_solved (p : ℕ) (hp : p.Prime) (n k : ℕ)
    (hn : 0 < n) (hk : 0 < k) :
    a (n * p ^ k) ≡ a (n * p ^ (k - 1)) [ZMOD (p : ℤ) ^ k] := by
  have hleft : n * p ^ k ≠ 0 :=
    Nat.ne_of_gt (Nat.mul_pos hn (Nat.pow_pos hp.pos))
...
theorem conjecture3Target_solved : conjecture3Target := by
  intro p hp n k hn hk
  exact conjecture3_solved p hp n k hn hk

#print axioms conjecture3_solved
#print axioms conjecture3Target_solved
